    ...
    <main className="max-w-4xl mx-auto px-6 py-16">
      ...
      <section className="mb-12">
        ...
        <ul className="space-y-2 text-ink/80 list-disc list-inside">
          <li>{t("sources.method1")}</li>
          <li>{t("sources.method2")}</li>
          <li>{t("sources.method3")}</li>
          <li>{t("sources.method4")}</li>
          <li>{t("sources.method5")}</li>
          <li>{t("sources.method6")}</li>
        </ul>
      </section>

      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {sourceList.map((source) => (
          <SourceBadge key={source.id} source={source} url={source.url} />
        ))}
      </section>
    </main>
  );
